...
BIG_TECH_KEYWORDS = ['google', 'microsoft', 'meta', 'apple', 'amazon', 'openai', 'nvidia', 'anthropic', 'sony', 'nintendo', 'ea', 'ubisoft', 'tencent']
...
def check_big_tech(summary, comment):
    text = f"{summary} {comment}".lower()
    return any(company in text for company in BIG_TECH_KEYWORDS)

def is_game_related(title, summary):
    """タイトルか概要にゲーム関連のキーワードが含まれるか判定"""
    keywords = ['game', 'games', 'gaming', 'gameplay', 'npc', 'procedural content generation', 'pcg', 'rl environment']
    text = f"{title} {summary}".lower()
...
def get_game_ai_papers(history):
    print("Fetching Game AI papers from arXiv...")
    # ゲームAIが最も多く投稿される cs.AI (Artificial Intelligence) をターゲットにします
    query = "cat:cs.AI"
    ...
    try:
        ...
        # タイムスタンプの基準を設定
        one_day_ago = now - timedelta(hours=36) # 時差や投稿タイミングを考慮し、約1.5日前までを「前日枠」とする
...
def call_llm(prompt):
    """【修正】gemini-3.1-flash-lite のみを使用"""
...
def main():
    ...
    candidates = get_game_ai_papers(history)
    
    if not candidates:
        print("配信対象（直近1ヶ月以内）の新しいゲームAI論文はありませんでした。")
        return
    ...
    prompt = f"""
    高度なゲームAIリサーチ専門家として振る舞ってください。提示されたすべての論文について、指定のJSON配列形式でのみ翻訳・解説を出力してください。
    ...
    [
      {{
        ...
        "tags": ["タグ1", "タグ2"],
        "layman_point": "ゲームAIの文脈でどこが凄いのか、専門外の人でもわかるポイント(日本語)",
        "interest_score": 10,
    ...
    if selected_results:
        ...
        # ヘッダー通知
        header = f"🎮 **【Game AI】本日({today})の厳選論文** (AI: {used_model})\n"
